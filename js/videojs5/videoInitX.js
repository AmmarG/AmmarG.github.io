...
(function() {
	var videlements = document.getElementsByClassName("video-js");
	for(var i=0; i<videlements.length; i++)
    {
		var idPostFixAtt = videlements[i].getAttribute('data-postfix');
		console.log("idPostFixAtt" +idPostFixAtt);
		initVideoJsAR('html5video', idPostFixAtt , '', '${pageCatVid!}/articles');
...
function initVideoJsAR(videoIdName, idPostFix , videoLable, pageCategoryVID) {
		...
		if(videlement==null){
			console.log('varMap vidplayer NULL NULL: '+ videoIdName+idPostFix + videlement);
		}
		console.log('varMap vidplayer: '+ videoIdName+idPostFix + videlement);
		var isInPlayMode=false;
